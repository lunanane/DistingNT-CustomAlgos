// Adapted from Mutable Instruments' plaits/dsp/drums/analog_snare_drum.h
// (MIT license, see ../../../ATTRIBUTION.md).
//
// Modification: same as analog_bass_drum.h - the "sustain" (drone) mode
// and its per-mode SineOscillator array are removed, since `sustain` is a
// runtime bool (both branches would always compile in) and we don't need
// that mode for a triggered drum machine.
//
    ...
    while (size--) {
      ...
      for (int i = 0; i < kNumModes; ++i) {
        float excitation = i == 0
            ...
            : 0.026f * pulse;
        shell += gain[i] * (resonator_[i].Process<stmlib::FILTER_MODE_BAND_PASS>(
      ...
      noise *= noise_envelope_ * snappy * 2.0f;
